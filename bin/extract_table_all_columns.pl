	...
	if (($w == $ws)||($col_final==250)) {
		...
		for my $col2 ( $col_min+1 .. $col_max ) {
			...
			if (!system($cmd)) {
			    ...
			    # check the series ID against the prior list of filenames
			    my $srce = "../file_list.txt";
			    my $checkstring = $series_ID->value;
			    open(my $FH, $srce) or die "Failed to open file $srce ($!)";
			    my @buf = <$FH>;
			    close($FH);
			    my @lines = grep (/$checkstring/, @buf);
			    ...
			    if ($numlines>0) {
				#print $lines[0];
				$filename_stem = basename($lines[0],  ".svg");
				$filename_stem =~ s/\R//g; #remove linebreak from end
				$filename_stem =~ s/\s/_/g; #spaces in filenames must be uploaded as _
				$filename_stem =~ s/ /_/g;
				$filename_stem =~ s/File://g;
				#print $filename_stem.".svg\n";
				#exit;
			    } else {
				#my $filename_stem = $output_string.($col_effective+1).'-'.$catalogue_title_short.'-'.$table_title_short.'-'.$cell_header_text;
				$filename_stem = $output_string.'-'.$catalogue_title_short.'-'.$table_title_short.'-'.$cell_header_text;
				$filename_stem = substr $filename_stem, 0, 226;
				$filename_stem = $filename_stem.'-'.$series_ID->value();

				open (ALLFILENAMES, '>>../file_list.txt');
				print ALLFILENAMES 'File:'.$filename_stem.'.svg'."\n";
				print 'added to file_list.txt: '.$filename_stem.'.svg'."\n";
				close(ALLFILENAMES);
			    }
				...
				if (!$duplicate) {
				    ...
				    print DESCRIPTIONS 'python ../../../pywikipedia/pwb.py upload.py -keep ';
				    print DESCRIPTIONS '-ignorewarn ';
				    print DESCRIPTIONS '-noverify ';
				    print DESCRIPTIONS $filename_stem.'.svg ';
				    print DESCRIPTIONS $description_text."\n";

				    print REUPLOAD 'python ../../../pywikipedia/pwb.py reupload_tsh.py -keep ';
				    print REUPLOAD '-ignorewarn ';
				    ...
				    print REUPLOAD 'python ../../../pywikipedia/pwb.py replace.py -always -summary:"full_description" -regex -page:File:';
				    print REUPLOAD $filename_stem.'.svg ';
				    print REUPLOAD '"update$" ';
				    print REUPLOAD $description_text."\n";
